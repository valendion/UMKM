...
class AdapterLocationPopular(val dataSet: ModelResponseMessage) :
    RecyclerView.Adapter<AdapterLocationPopular.ViewHolderLocationPopular>() {


    inner class ViewHolderLocationPopular(val binding: ItemDetailCategoryBinding) :
        RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolderLocationPopular {
        val binding = ItemDetailCategoryBinding
            .inflate(LayoutInflater.from(parent.context), parent, false)
        return ViewHolderLocationPopular(binding)
    }

    override fun onBindViewHolder(holder: ViewHolderLocationPopular, position: Int) {
        with(holder){
            binding.apply {
